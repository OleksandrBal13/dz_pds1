...
while True:
    ...
    print(total)
    conn.send(bytes("Total price of your order is " + str(total)+ " USD", encoding='UTF-8'))
    data3 = conn.recv(1024)
    lenght = len(data3.split())
    conn.send(bytes("There are " + str(lenght) + " words in your message", encoding='UTF-8'))
conn.close()